...
describe("Techo", async () => {
  ...
  // it("both owner and tenant have 5000 DAI each", async () => {
  ...
  it("Landlord collects 12 rents in a 12 month contract - happy path ", async function () {
    ...
    for (let i = 0; i < 12; i++) {
      const mapping = await techo.cycleMapping(i);
      console.log("mapping", mapping.index, mapping.paid, utils.formatEther(mapping.start), utils.formatEther(mapping.finish));
    }
    ...
    for (let cycleIndex = 0; cycleIndex < 12; cycleIndex++) {
      ...
      expect(mapping.paid).to.be.true;
      console.log("mapping", mapping.index, mapping.paid, mapping.start, mapping.finish);
      const landlordBalance = await dai.balanceOf(landlord.address);
      console.log("landlord balance", utils.formatEther(landlordBalance));
      //expect(utils.formatEther(landlordBalance)).to.equal("416.666666666666666666");

      cycleIndex++;
      console.log("latest before", utils.formatEther(await time.latest()));
      await time.increase(time.duration.weeks(4))
      console.log("latest after", utils.formatEther(await time.latest()));
      console.log("mapping", mapping.index, mapping.paid, mapping.start, mapping.finish);
